...
namespace Tiled.Droid
{
    public class GoldRush : CCLayer
    {
        ...
        CCLabel coin_count_label2;
        CCLabel coin_separator;
        ...
        public GoldRush(MainLayer mainLayer, String level, int hp, String speed, NetworkStream serverStream, int id, int playercount)
        {
            _level = level;
        ...
        protected void HandleIncomingEvent(String text, String ip)
        {
            ...
            switch (temp[0])
            {
                ...
                case "Victory":
                    ...
                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GLeave");
                ...
                case "DisconnectGame":
                    ...
                    break;
                case "DeleteCoin":
                    RemoveCoin(int.Parse(temp[1]), int.Parse(temp[2]));
                    break;
                default:
                    break;
            }
        ...
        {
            for(int i = coin_List.Count - 1; i >= 0; i--)
            {
                if ((coin_List[i].PositionX == x) && (coin_List[i].PositionY == y))
                {
                    this.RemoveChild(coin_List[i]);
                    coin_List.RemoveAt(i);
                    if (_id == 0)
                    {
                        coin_count++;
                        coin_count_label.Text = coin_count.ToString() + "       -  ";
                    }
                    else if (_id == 1)
                    {
                        coin_count2++;
                        coin_count_label2.Text = coin_count2.ToString();
                    }
                }
            }
        }
        ...
        protected override void AddedToScene()
        {
            ...
            coin_count = 0;
            coin_count_label = new CCLabel(coin_count.ToString(), "fonts/MarkerFelt", 22, CCLabelFormat.SpriteFont);
            coin_count_label.Scale = 0.5f;
            coin_count_label.Position = new CCPoint(172, 225);
            this.AddChild(coin_count_label);

            coin_separator = new CCLabel("-", "fonts/MarkerFelt", 22, CCLabelFormat.SpriteFont);
            coin_separator.Scale = 0.5f;
            coin_separator.Position = new CCPoint(197, 225);
            this.AddChild(coin_separator);

            coin_count2 = 0;
            coin_count_label2 = new CCLabel(coin_count2.ToString(), "fonts/MarkerFelt", 22, CCLabelFormat.SpriteFont);
            coin_count_label2.Scale = 0.5f;
            coin_count_label2.Position = new CCPoint(212, 225);
            this.AddChild(coin_count_label2);
        ...
        {
            time_label.Text = (60 - (DateTime.Now - date_start).Seconds).ToString();
            if ((DateTime.Now - date_start).TotalSeconds > 10)
            {
                _mainLayer.EndCoinGame(coin_count, coin_count2, 60, _id);
            }
        }
                ...
                if (_id == 0)
                {
                    foreach (var characterModel in charmodel_List)
                    {
                        if (coin.BoundingBoxTransformedToWorld.IntersectsRect(characterModel.BoundingBoxTransformedToParent))
                        {
                            ...
                            byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GCoin;" + coin.PositionX.ToString() + ";" + coin.PositionY.ToString());
                            _serverStream.Write(outStream, 0, outStream.Length);
                            _serverStream.Flush();
                            RemoveChild(coin);
        ...
        private void HandleInput(System.Collections.Generic.List<CCTouch> touches, CCEvent touchEvent)
        {
            if (touches.Count > 0)
            {
                foreach (var touch in touches)
                {
                    ...
                    {
                        byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GRight button Request");
                        _serverStream.Write(outStream, 0, outStream.Length);
                        _serverStream.Flush();
                    }
                    else if (button_left.sprite.BoundingBoxTransformedToWorld.ContainsPoint(touch.Location))
                    {
                        byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GLeft button Request");
                        _serverStream.Write(outStream, 0, outStream.Length);
                        _serverStream.Flush();
                    }
                    ...
                    {
                        byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GUp button Request");
                        _serverStream.Write(outStream, 0, outStream.Length);
                        _serverStream.Flush();
                    }
                    ...
                    {
                        byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GDown button Request");
                        _serverStream.Write(outStream, 0, outStream.Length);
                        _serverStream.Flush();
                    }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0] - 1).ToString() + ";" + (char_pos[_id][1]).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0]).ToString() + ";" + (char_pos[_id][1] - 1).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0]).ToString() + ";" + (char_pos[_id][1] + 2).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0] + 2).ToString() + ";" + (char_pos[_id][1]).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0] + 1).ToString() + ";" + (char_pos[_id][1] + 1).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
                        ...
                        foreach (var door in door_List)
                        {
                            if (door.BoundingBoxTransformedToWorld.ContainsPoint(location))
                            {
                                if (door_key_count > 0)
                                {
                                    ...
                                    byte[] outStream = System.Text.Encoding.ASCII.GetBytes("GOpen;" + (char_pos[_id][0] + 1).ToString() + ";" + (char_pos[_id][1] + 2).ToString());
                                    _serverStream.Write(outStream, 0, outStream.Length);
                                    _serverStream.Flush();
                                }
                            }
